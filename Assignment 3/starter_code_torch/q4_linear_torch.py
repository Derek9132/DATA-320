...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        tensor = torch.flatten(state, start_dim = 1)
    ...
    def calc_loss(
        ...
        q_values_for_actions = torch.sum(q_values * F.one_hot(actions.long(), num_classes=num_actions), dim=1)
        
        target_q_samp = rewards + gamma * torch.max(target_q_values, dim=1).values * torch.bitwise_not(done_mask)

        loss = F.mse_loss(q_values_for_actions, target_q_samp)
