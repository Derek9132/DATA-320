...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        """
        state_shape = self.env.state_shape() # 2 dimensional shape
        ...
        input_size = (img_height - 2) * (img_width - 2) * 16

        model = nn.Sequential (nn.Conv2d(n_channels * self.config.state_history, 16, 3, 1), # convolutional layer: 16 output channels, kernel size 3, stride 1
                                nn.ReLU(), 
                                nn.Flatten(1), 
        ...
        model_target = nn.Sequential (nn.Conv2d(n_channels * self.config.state_history, 16, 3, 1), # convolutional layer: 16 output channels, kernel size 3, stride 1
                                nn.ReLU(), 
                                nn.Flatten(1), 
                                nn.Linear(input_size, 128), 
                                nn.ReLU(), 
        ...
        self.target_network = model_target
